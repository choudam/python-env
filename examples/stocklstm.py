...
epochs_num=100
#layer 1 (input layer)
regressor.add(LSTM(units=hiddenunits,return_sequences=True,input_shape=(X_train.shape[1],X_train.shape[2])))
...
print(y_train_close)
regressor.fit(X_train,y_train_close,epochs=epochs_num,batch_size=32)
...
#print(y_pred1)
y_pred1[:,3]=y_pred.reshape(y_pred.shape[0])
#print(y_pred1)

#graph predicted stock price

y_test_price = sc.inverse_transform(pd.DataFrame(y_test))[:,3]
y_pred_price=sc.inverse_transform(pd.DataFrame(y_pred1))[:,3]
